        ...
        <div className="w-full max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
          {education.map((edu, idx) => (
            <motion.a 
              ...
              className="group relative glassCard rounded-[2.5rem] p-8 md:p-12 flex flex-col items-center text-center shadow-sm hover:shadow-xl transition-all duration-500"
            >
              {/* Logo Container */}
              <div className={`w-28 h-28 md:w-36 md:h-36 shrink-0 bg-white rounded-3xl flex items-center justify-center overflow-hidden border border-[#eee] shadow-md transition-all duration-500 relative z-10 mb-8 ${edu.id === 2 ? 'p-3' : ''}`}>
              ...
              {/* Typography */}
              <h3 className="text-2xl md:text-4xl font-black text-[#111] tracking-tight mb-4 transition-colors relative z-10">{edu.institution}</h3>
              
              <div className="text-lg md:text-xl font-bold text-[#555] mb-8 relative z-10">{edu.title}</div>
              ...
              {/* Meta details */}
              <div className="flex flex-col gap-4 w-full relative z-10 mt-auto bg-[#fafafa] p-6 rounded-3xl border border-[#f0f0f0] group-hover:border-[#e0e0e0] group-hover:bg-white transition-colors duration-500">
                <div className="flex items-center justify-center gap-3 text-sm md:text-base font-black tracking-widest uppercase text-[#555]">
                  <Calendar size={18} strokeWidth={2.5} />
                  {edu.duration}
                </div>
                <div className="flex items-center justify-center gap-3 text-sm md:text-base font-medium text-[#666]">
